...
export function NewPageForm({
  ...
  const domainOptions = useMemo(
    () => buildNewPageDomainOptions(domainCatalog, t("otherPagesFlat")),
    [domainCatalog, t],
  );
  ...
  const handleAddDomain = async () => {
    ...
    if (!validation.valid || !validation.normalized) {
      setAddDomainError(validation.error ?? t("errorDomainInvalid"));
      return;
    }
    ...
    try {
      ...
      setAddDomainError(err instanceof Error ? err.message : t("errorDomainAddFailed"));
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!validation.valid) {
        setError(validation.error ?? t("errorSlugGeneric"));
        return;
      }
    ...
    <form className="page-manager-fab__form" onSubmit={(event) => void handleSubmit(event)}>
      ...
      <div className="page-manager-fab__field">
        ...
        <p id="new-page-path-preview" className="page-manager-fab__path-preview">
          {t("pathPreview", { path: previewPath })}
        </p>
      </div>
      ...
      <Button type="submit" className="page-manager-fab__submit" disabled={submitting}>
        {submitting ? t("opening") : t("openEditor")}
      </Button>
    </form>
  );
}
