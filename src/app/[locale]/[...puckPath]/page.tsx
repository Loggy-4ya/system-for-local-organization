...
export default async function PuckPage({
  ...
  if (puckPath.length === 1 && puckPath[0] === "edit") {
    return await redirect("/pages/edit?error=homepage-code-only");
  }
  ...
  if (isEditing && path === "/edit") {
    return await redirect("/pages/edit?error=reserved-slug");
  }
  ...
  if (isEditing && !canEdit) {
    return await redirect(resolveUnauthorizedEditorRedirectPath(path));
  }
