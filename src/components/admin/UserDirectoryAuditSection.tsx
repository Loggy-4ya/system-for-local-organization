 ...
 */
function formatActionLabel(action: UserDirectoryAuditRow["action"]): string {
  if (action === "user_delete") return "Delete";
  return "Update";
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="max-w-3xl text-sm leading-relaxed text-(--color-text-secondary)">
        Records admin changes from the User Directory: profile edits, hierarchy moves, delegated
        permissions, socium labels, and account deletions. Payloads store field keys and safe
        metadata only — no email, phone, or login handles of the target user.
      </p>
      ...
      >
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-(--color-text-secondary)">
            No user directory events recorded yet.
          </div>
        ) : (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant={row.success ? "secondary" : "destructive"}>
                        {row.success ? "Success" : "Rejected"}
                      </Badge>
                      <Badge variant="outline">{formatActionLabel(row.action)}</Badge>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-(--color-text-secondary)">
                      ...
                      {row.actorLogin ? <span>Actor: {row.actorLogin}</span> : null}
                      {row.targetDisplayName ? (
                        <span>Target: {row.targetDisplayName}</span>
                      ) : (
                        <span className="font-mono">Target id: {row.targetUserId}</span>
                      )}
                    </div>
                  </button>
                  ...
                  {isExpanded ? (
                    <div className="border-t border-border bg-muted/20 px-5 py-4">
                      <dl className="grid gap-3 text-xs sm:grid-cols-2">
                        <div>
                          <dt className="font-medium text-(--color-text-secondary)">Actor id</dt>
                          <dd className="font-mono text-(--color-text-primary)">{row.actorUserId}</dd>
                        </div>
                        <div>
                          <dt className="font-medium text-(--color-text-secondary)">Target id</dt>
                          <dd className="font-mono text-(--color-text-primary)">{row.targetUserId}</dd>
                        </div>
                        {row.errorCode ? (
                          <div>
                            <dt className="font-medium text-(--color-text-secondary)">Error code</dt>
