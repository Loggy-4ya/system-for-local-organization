...
| `npm run test:desktop-letterbox-zoom` | `tests/puck/lib/desktopLetterboxZoom.test.ts` | `sanitizePuckZoomConfig.ts` | Fixed presets floor auto-fit zoom at 1× (horizontal scroll when preset wider than canvas; letterbox when narrower). |
