...
- `applyBlockShell()` — margin shell + optional centered island wrapper with preset fill/border. **New** island-default blocks seeded with **Small (8px)** top/bottom margin on insert/move only (`mergeIslandAutoSpacing`); saved/starter content is not retroactively patched.
...
| Fix | Files | Detail |
|-----|-------|--------|
| Carousel edit navigation | [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`puck-editor.css`](../../src/app/puck-editor.css) | Edit arrows/dots wrap infinitely (`goToSlide` modulo) — Embla `loop` stays off to avoid clone nodes breaking Puck slots. Drop zones fill the full stretched slide height (`flex` chain on slide + slot). |
...
| Fix | Files | Detail |
|-----|-------|--------|
| Slides per view | [`NexusCarousel.tsx`](../../src/components/puck/blocks/content/NexusCarousel.tsx), [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`carouselEmblaOptions.ts`](../../src/components/puck/lib/carouselEmblaOptions.ts), [`puck-editor.css`](../../src/app/puck-editor.css), [`globals.css`](../../src/app/globals.css) | **Slides Visible** applies in edit and interactive modes (2/3/auto). **spv-1** (and mobile **auto**) use `nexus-carousel--edit-single-slide`: inactive slides collapse to zero width, Embla track transform is pinned to zero, dots/arrows only change `activeSlideIndex` (no `scrollTo`). When every slide fits (`nexus-carousel--edit-static-fit`), same CSS-only model. When slides overflow, edit dots/arrows use interactive **page snaps**. **Overflow:** clip on edit root + Embla viewport only. |
...
| Slides per view CSS | [`globals.css`](../../src/app/globals.css) | Root classes `nexus-carousel--spv-1|2|3|auto` set `flex-basis` with loop-safe spacing (`padding-left` + negative track margin + `width: calc(100% + gap)` so the row fills the viewport — not flex `gap`, which breaks at the loop seam). `nexus-carousel--single-frame` (1 visible) vs `nexus-carousel--multi-slide` (2+ visible) — multi-slide renders **separate bordered cards** per slide, not one outer panel. Nested video blocks and images with carousel fill use `nexus-carousel__slide-media-fill` to cover the slide card in edit and interactive modes. |
| Scroll step | [`NexusCarousel.tsx`](../../src/components/puck/blocks/content/NexusCarousel.tsx), [`carouselEmblaOptions.ts`](../../src/components/puck/lib/carouselEmblaOptions.ts) | **Scroll Step** sidebar field: advance 1 / 2 / 3 slides or **Full page** (matches visible count; `auto` uses responsive breakpoints). Embla `slidesToScroll` updated via `reInit` — no carousel remount. |
| Anti-flicker sizing | [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`carouselMediaFill.ts`](../../src/components/puck/lib/carouselMediaFill.ts), [`puck-editor.css`](../../src/app/puck-editor.css), [`selectPuckComponentById.ts`](../../src/components/puck/lib/selectPuckComponentById.ts) | Edit track height locked to **tallest slide** via `--nexus-carousel-edit-height` (`ResizeObserver`). Per slide: in-flow blocks sum `offsetHeight`; fill-slide media contributes `width × aspect ratio` (`data-nexus-media-aspect`); slide height is `max(stack, fill)`. Carousel height is the max across all slides (including collapsed single-slide siblings via fallback width). **Carousel settings** button + shell click select the carousel block programmatically. |
...
| Island user override | [`spacingFields.tsx`](../../src/components/puck/lib/spacingFields.tsx), [`applyIslandDefaultsOnInsert.ts`](../../src/components/puck/lib/applyIslandDefaultsOnInsert.ts), [`IslandInsertDefaultsSync.tsx`](../../src/components/puck/IslandInsertDefaultsSync.tsx), [`IslandFieldGroup.tsx`](../../src/components/puck/fields/IslandFieldGroup.tsx), [`puckDataTree.ts`](../../src/components/puck/lib/puckDataTree.ts) | `islandUserOverride: true` when user toggles island. **New root-level inserts** (any block type, including Carousel) get **Small (8px)** top/bottom margin via `shouldSeedRootInsertSpacing`; palette `defaultProps.spacing` seeds the same. Island-default types also auto-enable island. `pendingInsertIds` retries margin seeding after Puck async `resolveData` → `replace`. Nested inserts under an active island/slot shell reset margins to **None**. Saved pages and the welcome section are **not** auto-margin-healed on load. Blocks inserted into `NexusCarousel` / `NexusTabs` slots auto-set `islandEnabled: false` (unless user override). `normalizeNestedIslands()` strips redundant child islands under slot shells and parent islands, but respects `islandUserOverride`. Island toggle stays **enabled** in carousel/tab slides with an optional hint — only nested-under-active-island remains disabled. |
...
| Preset | Behavior |
|--------|----------|
| `1` | Advance one slide per arrow click / autoplay tick |
| `2` / `3` | Advance 2 or 3 slides |
