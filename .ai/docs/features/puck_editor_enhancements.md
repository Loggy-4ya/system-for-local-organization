...
- **Header chrome:** Removed from canvas preview. Editor uses unified `NexusPuckHeaderShell` (`overrides.header`) — glass-panel bar matching `SiteHeaderBar` (Nexus logo + Puck toolbar). Published `<Render>` uses `GlobalHeader` from `layout.tsx`. `PageRoot` still renders contained `InfiniteGrid` in edit/interactive preview via `NexusEditorCanvasContext`.
...
| HTML sanitize/render | `richTextContent.ts` | Allowlist sanitizer + legacy plain-text fallback for `NexusText` canvas/publish. SSR uses a string sanitizer (no `linkedom`) so HTML is not double-escaped and the client bundle stays browser-safe. |
...
| Fix | Files | Detail |
|-----|-------|--------|
| Carousel edit navigation | [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`puck-editor.css`](../../src/app/puck-editor.css) | Edit: single-slide / static-fit cycle slides via modulo on `activeSlideIndex`; multi-slide overflow stops at first/last page. Embla `loop` stays **off** in edit (no clone nodes). **Interactive/published only:** Embla `loop` + arrows/dots/autoplay wrap at page boundaries. |
...
| Selection overlay height | [`puck-editor.css`](../../src/app/puck-editor.css), [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx) | All slides stay in the Embla track so carousel **auto height equals the tallest slide**. Inactive edit slides only disable pointer events (no height collapse). `ResizeObserver` + Puck node sync on slide / resize change. |
...
| Anti-flicker sizing | [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`carouselMediaFill.ts`](../../src/components/puck/lib/carouselMediaFill.ts), [`puck-editor.css`](../../src/app/puck-editor.css), [`selectPuckComponentById.ts`](../../src/components/puck/lib/selectPuckComponentById.ts) | Edit track height locked to **tallest slide** via `--nexus-carousel-edit-height` (`ResizeObserver`) when slides mix grids or in-flow blocks. **Fill-only slides** skip the CSS variable — `aspect-ratio` on media defines height. Per slide: in-flow blocks sum `offsetHeight`; fill-slide media contributes `width × aspect ratio` (`data-nexus-media-aspect`); slide height is `max(stack, fill)`. Carousel height is the max across all slides (including collapsed single-slide siblings via fallback width). **Carousel settings** button + shell click select the carousel block programmatically. |
...
| Slot render pattern | [`NexusGrid.tsx`](../../src/components/puck/blocks/layout/NexusGrid.tsx), [`NexusGridItem.tsx`](../../src/components/puck/blocks/layout/NexusGridItem.tsx) | Grid container slot uses `className="nexus-grid"` + `minEmptyHeight` in edit mode. Grid item slot receives `puck.dragRef`, span styles, and `className="nexus-grid-item"` directly on the Puck slot (dropzone) element. |
...
| Drop-zone affordances | [`puck-editor.css`](../../src/app/puck-editor.css) | `.nexus-grid` / `.nexus-grid-item` get min-height, padding, dashed accent outlines on empty/hover/destination states; nested components fill cell width. |
...
| Grid height feedback loop | [`globals.css`](../../src/app/globals.css), [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`NexusGridItem.tsx`](../../src/components/puck/blocks/layout/NexusGridItem.tsx) | Carousel flex/`height:100%` rules no longer apply to nested grid drop-zones; `measureGridLayoutHeight()` measures intrinsic size; ResizeObserver watches slides only; removed `minEmptyHeight` from grid items. |
| Grid + auto carousel | [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`NexusGrid.tsx`](../../src/components/puck/blocks/layout/NexusGrid.tsx), [`globals.css`](../../src/app/globals.css), [`puck-editor.css`](../../src/app/puck-editor.css) | When a slide contains `.nexus-grid`, carousel gets `nexus-carousel--has-grid-slide` and **skips** `--nexus-carousel-edit-height` sync; track/viewport/slides use `height:auto`. Grid uses `gridAutoRows:min-content` + `alignItems:start`; removed grid `minEmptyHeight`. |
...
| Unified editor header | [`NexusPuckHeaderShell.tsx`](../../src/components/puck/NexusPuckHeaderShell.tsx), [`puckEditorOverrides.tsx`](../../src/components/puck/puckEditorOverrides.tsx), [`puck-editor.css`](../../src/app/puck-editor.css) | Puck `overrides.header` wraps toolbar in `SiteHeaderBar`-style glass panel. Desktop: logo + sidebar toggle buttons + centered title/slug + MenuBar/actions on one flex row (`align-items: center`); **Puck `PuckHeader-toggle` must stay visible on desktop** (left/right sidebar hide buttons). Mobile (≤900px): compact single row — logo left, undo/redo + theme + All Pages + Interactive + Publish right; page title/slug and sidebar toggles hidden; `MenuBar` forced `position: static` (no absolute dropdown row). Theme switch uses square-rounded corners globally (`globals.css`). |
...
| Mobile ≤900px | same | Bottom **plugin rail** + visible **left plugin panel** — top-rounded bordered islands; layout inner horizontal inset; canvas stays fullScreen (no outer panel chrome) |
...
| Canvas drag coordinator | [`NexusCanvasDragCoordinator.tsx`](../../src/components/puck/NexusCanvasDragCoordinator.tsx), [`puckEditorOverrides.tsx`](../../src/components/puck/puckEditorOverrides.tsx) | Preview iframe overlay highlights + post-drop corrective `move`/`reorder` dispatch. Mount via `overrides.puck`. |
| Slot class markers | [`NexusSection.tsx`](../../src/components/puck/blocks/layout/NexusSection.tsx), [`NexusColumns.tsx`](../../src/components/puck/blocks/layout/NexusColumns.tsx), [`NexusGrid.tsx`](../../src/components/puck/blocks/layout/NexusGrid.tsx) | Edit mode: `nexus-section__dropzone`, `nexus-columns__dropzone`; grid container `minEmptyHeight={120}`. Grid items unchanged (avoid height loop). |
...
| Spec | [`puck_canvas_drag_drop.md`](./puck_canvas_drag_drop.md) | Slot inventory, architecture diagram, developer checklist, troubleshooting. |
