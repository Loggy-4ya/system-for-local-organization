...
| Tests | `npm run test:run -- island-defaults` — root move seeds non-`none` vertical margins |
...
| Feature | File | Detail |
|---------|------|--------|
| Edit / Interactive toggle | `EditorModeToggle.tsx`, `editorModePanelSnapshot.ts` | Toggles Puck `ui.previewMode` via `setUi`; entering interactive closes plugin panels; returning to edit restores prior sidebar visibility via in-memory snapshot (`npm run test:run -- editor-mode-panel`) |
...
| List stepper + highlights | [`ListStepLabelField.tsx`](../../src/components/puck/fields/ListStepLabelField.tsx), [`ListItemTiptapField.tsx`](../../src/components/puck/fields/ListItemTiptapField.tsx), [`ListStepChildrenField.tsx`](../../src/components/puck/fields/ListStepChildrenField.tsx), [`ListConnectorStyleField.tsx`](../../src/components/puck/fields/ListConnectorStyleField.tsx), [`listStepTree.ts`](../../src/components/puck/lib/listStepTree.ts), [`NexusStepperTimeline.tsx`](../../src/components/ui/NexusStepperTimeline.tsx), [`NexusList.tsx`](../../src/components/puck/blocks/content/NexusList.tsx), [`globals.css`](../../src/app/globals.css), [`puck-editor.css`](../../src/app/puck-editor.css) | Semantic vertical timeline. Connection styles: `spine`, `dashed`, `dotted`, `tree`, `rail`, `ring`, `arrow`, `minimal` (legacy `segment` → `dashed`). Style chapter uses visual picker grid. Sub-step TipTap uses full-width host (no accordion squeeze). |
...
| Anti-flicker sizing | [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`carouselMediaFill.ts`](../../src/components/puck/lib/carouselMediaFill.ts), [`puck-editor.css`](../../src/app/puck-editor.css), [`globals.css`](../../src/app/globals.css), [`selectPuckComponentById.ts`](../../src/components/puck/lib/selectPuckComponentById.ts) | **Multi-slide edit + interactive:** dynamic equal row via CSS flex stretch (`equal-row-height`); no `--nexus-carousel-row-height` lock. **Edit** uses `--nexus-carousel-edit-min-height` floor; **interactive/published** uses `--nexus-carousel-row-min-height` (defaults to `--nexus-carousel-auto-min-height`, **240px**). **Inside grid cells** (`nexus-carousel--in-grid-cell`), row floor inherits **`--nexus-grid-item-empty-min-height`** from the parent grid shell (**240px** page grid / **120px** inside carousel slides — see [`gridEditSizing.ts`](../../src/components/puck/lib/gridEditSizing.ts)). **Single-slide edit:** per-slide `--nexus-carousel-slide-height`. **Single-visible interactive/phone:** active-slide row sync only. **Carousel settings** button + shell click select the carousel block programmatically. |
...
| Drop-zone affordances | [`puck-editor.css`](../../src/app/puck-editor.css), [`NexusGridRender.tsx`](../../src/components/puck/blocks/layout/NexusGridRender.tsx), [`usePuckOverlayPortal.ts`](../../src/components/puck/lib/usePuckOverlayPortal.ts) | `.nexus-grid` / `.nexus-grid-item` min-height + dashed outlines; grid item edit hides Puck append hitboxes and flex-fills the cell (240px empty floor, 120px in carousel slides). **Grid settings** gear (`.nexus-grid__edit-select`) shows in **edit mode** whenever the grid is on canvas. **Empty cell click-to-select:** register Puck overlay portal on the gear button only — not the full-bleed `.nexus-grid__controls` wrapper (`inset: 0`), because Puck CSS forces `[data-puck-overlay-portal] { pointer-events: auto !important }` and would block cell hits. Wrapper stays `pointer-events: none !important`; cell shell / dropzone clicks call `selectPuckComponentById`. **Grid Cells sidebar:** top-level `items` array uses compact Fields-plugin array rhythm (single gutter pass, flat add row — see `puck-editor.css` FieldsPlugin array overrides). |
...
| Fix | Files | Detail |
|-----|-------|--------|
| Composite fill guard | [`carouselMediaFill.ts`](../../src/components/puck/lib/carouselMediaFill.ts), [`CarouselSlideMediaContext.tsx`](../../src/components/puck/CarouselSlideMediaContext.tsx), [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`NexusVideoRender.tsx`](../../src/components/puck/blocks/content/NexusVideoRender.tsx), [`NexusImageRender.tsx`](../../src/components/puck/blocks/content/NexusImageRender.tsx), [`globals.css`](../../src/app/globals.css) | `resolveCarouselSlideCompositeFlags()` reads Puck private zone indexes (`contentIds` per `slides[i].content`) so **Input + Video** stacks suppress `auto` fill-slide on first paint. Slides with composite layout get `nexus-carousel__slide--composite` for content-sized drop zones while the carousel row keeps **`nexus-carousel--equal-row-height`** / **240px** viewport floor in interactive multi-slide view. Carousel-in-grid-item still fill-slides normally. |
...
| Tests | [`embedMedia.test.ts`](../../tests/puck/lib/embedMedia.test.ts) | `npm run test:run -- embed-media` — URL parsing, embed aspect lock, fit normalization. |
...
| Phone single-slide height | [`NexusCarouselRender.tsx`](../../src/components/puck/blocks/content/NexusCarouselRender.tsx), [`carouselMediaFill.ts`](../../src/components/puck/lib/carouselMediaFill.ts), [`puck-editor.css`](../../src/app/puck-editor.css), [`globals.css`](../../src/app/globals.css) | **`--nexus-carousel-row-height`** (edit single-visible row sync) and **`--nexus-carousel-slide-height`** (per-slide edit sync). **Multi-slide edit + interactive:** CSS flex stretch (`equal-row-height`, content-adaptive). **Single-slide edit:** per-slide sync on active slide. **Interactive/published auto-height:** no JS row-height lock — drop zones stay content-sized (no inline `height: 100%` stretch). Grid-in-slide carousels skip row sync in published mode (feedback-loop guard). |
...
| Responsive canvas overlap fix | [`usePuckMobileEditorChrome.ts`](../../src/components/puck/usePuckMobileEditorChrome.ts), [`puck-editor.css`](../../src/app/puck-editor.css), [`resolveAutoViewport.ts`](../../src/components/puck/lib/resolveAutoViewport.ts), [`puckCanvasSelectors.ts`](../../src/components/puck/lib/puckCanvasSelectors.ts), [`NexusEditorScrollportGrid.tsx`](../../src/components/puck/NexusEditorScrollportGrid.tsx), [`canvasLetterboxScrollport.ts`](../../src/components/puck/lib/canvasLetterboxScrollport.ts), [`interactivePreviewScrollport.ts`](../../src/components/puck/lib/interactivePreviewScrollport.ts) | Three-tier layout: compact ≤900px, narrow desktop 901–1023px, full desktop ≥1024px. Desktop ≥901px: unified `grid-template-columns` via `--nexus-grid-left-width` / `--nexus-grid-right-width` (0 when sidebar hidden, Puck var when visible) + literal `minmax(0, 1fr)` editor track; hidden sidebars + resize handles forced `width: 0`. Canvas shell `width: 100%` + `overflow-y: scroll` (vertical scrollbar pinned to the panel right edge) + scrollport grid on shell; `.PuckCanvas-inner` is horizontal overflow only (`overflow-y: hidden`). `resetEditorCanvasScrollports` + `syncCanvasInnerHorizontalScrollOrigin` clear stale `scrollLeft` on entry so letterboxed previews do not clip the page start. |
...
| Feature | Files | Detail |
|---------|-------|--------|
| Drop-target metrics | [`canvasDropTargetLogic.ts`](../../src/components/puck/lib/canvasDropTargetLogic.ts), [`canvasDropTargetLogic.test.ts`](../../tests/puck/lib/canvasDropTargetLogic.test.ts) | Pure resolver: empty slots use `max(container, dragged)` height; append slots reserve space below children; `80px` floor; deepest-zone hit-test. Tests: `npm run test:run -- canvas-drop-target`. |
| Canvas reparent logic | [`canvasReparentLogic.ts`](../../src/components/puck/lib/canvasReparentLogic.ts), [`canvasReparentLogic.test.ts`](../../tests/puck/lib/canvasReparentLogic.test.ts) | Destination index, descendant guard, move/reorder commit builder. Tests: `npm run test:run -- canvas-reparent`. |
