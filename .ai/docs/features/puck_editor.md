...
- **Production:** `MEDIA_STORAGE_DRIVER=s3` on AWS EC2 — see `.ai/docs/features/media_storage.md`
...
**See also:** [puck_editor_enhancements.md §6](./puck_editor_enhancements.md#6-block-spacing--island-layout) · `npm run test:run -- island-defaults`
...
- **Single grid (edit mode)** — `#nexus-bg` in root `layout.tsx` is the only grid. Puck runs with `iframe={{ enabled: false }}` so the preview renders inline in the shell document (no `srcDoc` iframe compositor). Canvas layers stay transparent so that grid shows through the whole editor.
...
- **`PuckIframeTheme.tsx`** — Legacy iframe override when iframe mode is enabled; injects tokens + transparent preview-document CSS after `CopyHostStyles`. Not used when `iframe.enabled` is false.
- **`infiniteGridPaintTheme.ts`** — Grid paint follows `document.documentElement[data-theme]` when set (not only `useTheme().resolvedTheme`).
- **`NexusEditorScrollportGrid.tsx`** — Compact layout CSS var sync only; does not mount InfiniteGrid.
...
**Tests:** `npm run test:run -- canvas-drop-target` · Full spec: [puck_canvas_drag_drop.md](./puck_canvas_drag_drop.md)
...
**Device preset scale mode:** Fixed presets change responsive layout inside the preview at the true device width (360 / 768 / 1280). `NexusPuckZoomGuard` via `floorLetterboxDevicePreviewZoom`: when the canvas frame is **narrower** than the preset, `resolveShrinkToFitDeviceZoom` scales the preview down (`frameWidth / presetWidth`) so it does not overlap sidebars; when the frame is **wider** than the preset, auto-fit zoom scales up moderately (capped at **1.42×**, targeting **72%** of frame width) so narrow phone presets fill the canvas without overshooting. `applyPuckCanvasRootZoomPresentation` re-applies zoom presentation when mobile preview clear helpers strip React-owned inline styles without a Puck re-render (editor entry with both sidebars open). **Inline preview (`iframe.enabled: false`):** page content sits in `#nexus-puck-preview-scale-host` with CSS `zoom` (not `transform: scale`) so block layout tracks the visual scale and section overflow does not crop text; `[data-puck-overlay]` portals stay unscaled siblings of the scale host. **Iframe preview:** `transform: scale(zoom)` on `#puck-canvas-root`. **Interactive preview scrollbars:** tall pages scroll inside `#preview-frame` (`PuckIframeTheme`); the canvas shell and `.PuckCanvas-inner` stay non-scrollable with `scrollbar-gutter: auto` so no empty gutter stripe overlaps page content. Interactive layout drops outer `PuckLayout-inner` padding/gap and stretches the canvas shell plus `#puck-canvas-root` (`top: 0`, `bottom: 0`) to the full inner height below the Puck header; viewport preset controls float on the host overlay. `syncInteractivePreviewIframeDocumentViewport` locks iframe `html`/`body`/`#frame-root` `min-height` to `#preview-frame` client height so PageRoot `min-height: 100%` does not collapse and leave a dead band inside the bordered shell. First-line content clears the controls via `--nexus-preview-controls-inset` padding on `.global-layout-page-content-slot` inside the iframe. Edit-mode shell uses `overflow-y: auto` + `scrollbar-gutter: auto` (not `stable`) for the same reason. `canvasLetterboxScrollport.ts` expands the inner scrollport and chains wheel events to the canvas shell when scaled previews overflow vertically. Manual zoom below 100% via the toolbar still works when Puck is not auto-shrinking. **Edit-mode canvas bounds:** `PageRoot` stays content-sized; `previewContentHeight.ts` syncs Puck `rootHeight` to `max(block intrinsic height + edit-only 48px overlay pad, bordered canvas shell viewport)` so sparse pages fill the panel while the InfiniteGrid shows through transparent `#puck-canvas-root` / preview iframe shells (`puck-editor.css` clears Puck 0.21's default white canvas root). **Interactive preview** locks `rootHeight` to the canvas viewport (`shellHeight / zoom`) so tall pages scroll inside the iframe (`overflow-y: auto` via `PuckIframeTheme`) — not via letterbox shell expansion. Letterbox `.PuckCanvas-inner` expansion stays **edit-only**. Tests: `npm run test:run -- preview-content-height`.
